...
public class ClientTCPStreamingHandler extends Thread {
	...
	public ClientTCPStreamingHandler(Socket s, DataInputStream dis, DataOutputStream dos) {
		this.client = s;
		clientInput = dis;
		clientOutput = dos;
			...
			while (true) {
				...
				try {
					...
					if (temporaryReadValue != null) {
						...
						System.out.println("SERVER thread (stream)----> File information received...");
						...
						System.out.println("SERVER thread (stream)----> File data received...");
						...
						if ("closedSession".equals(session)) {
							System.out.println("SERVER thread (stream)----> Client's output stream is shutdown");
							...
							System.out.println("SERVER (stream) SAYS: Client " + client + " closed.");
							System.out.println("SERVER (stream) SAYS: Used protocol is TCP");
							System.out.println("SERVER (stream) SAYS: the number of messages received from client is "
									+ nrOfReceivedMessages);
							System.out.println("SERVER (stream) SAYS: the number of bytes read from client is "
									+ clientMessageLength);
							System.out.println(
									"SERVER (stream) SAYS: the number of bytes written in file is " + bytesWritten);
